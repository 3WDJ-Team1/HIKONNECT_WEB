    ...
    <div>
        <vue-event-calendar
            ...
            <v-card v-for="(event, i) in hikingEvents" :key="i">
                ...
                <v-card-actions>
                    <v-btn flat @click="joinPlan(event.no)">JOIN</v-btn>
...
<script>
    ...
    export default {
        ...
        methods: {
            joinPlan(no) {
                axios.post('http://172.26.2.88:8000/api/enter_schedule',   {
        ...
        created() {
            axios.get(this.$HttpAddr + '/schedule/' + this.$route.params.groupid)
                .then((response) => {
